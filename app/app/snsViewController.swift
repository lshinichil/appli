...
class snsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        //        // アクションの追加
        self.refreshControl.addTarget(self, action: #selector(snsViewController.refreshData), for: .valueChanged)
        //        // snsTableViewに追加
   ...
   //     let timeLineCell = self.snsTableView.dequeueReusableCell(withIdentifier: "CosutomTableViewCell", for: indexPath) as! CosutomTableViewCell
        
        let timeLineCell = self.snsTableView.dequeueReusableCell(withIdentifier: "CosutomTableViewCell",for: indexPath) as! CosutomTableViewCell
        
        // カスタムセルの中のtimeLineLabelという名前のLabelにテキストを入力する
        /********** レッスン2 mBaasから取得したデータをセルのラベルにいれる **********/
        timeLineCell.TextLabel.text = self.data[indexPath.row].object(forKey: "timeLineMessage") as! String
        /********** レッスン2 mBaasから取得したデータをセルのラベルにいれる **********/
        ...
        file.getDataInBackground { (data, error) in
            ...
            if error == nil {
                // セルの画像を設定する
                timeLineCell.ImageView.image = UIImage(data: data!)
